...
async function loadChannels() {
  ...
  channelList.innerHTML = ` <div style="text-align:center;padding:30px"> ⏳ Loading Channels... </div>`;
...
function renderFeaturedChannels() {
  ...
  featured.forEach((channel) => {
    ...
    card.innerHTML = ` <img src="${ channel.logo || "logo.png" }" onerror="this.onerror=null; this.src='logo.png';"> <h4>${channel.name}</h4> <p>${channel.category}</p> `;
...
function renderChannels(list = channels) {
  ...
  if (filtered.length === 0) {
    channelList.innerHTML =
      "<p style='text-align:center;padding:20px;'>No Channel Found</p>";
    return;
  }
  ...
  filtered.forEach((channel) => {
    ...
    card.innerHTML = ` <img src="${ channel.logo || "logo.png" }" onerror="this.onerror=null; this.src='logo.png';"> <div class="info"> <h3>${channel.name}</h3> <p>${channel.category} ${ channel.country ? "• " + channel.country : "" }</p> <span class="live-badge">🔴 LIVE</span> </div> <button class="fav-btn"> ${isFav ? "❤️" : "🤍"} </button> `;
...
function setupEventListeners() {
  ...
  if (favoriteNav) {
    favoriteNav.onclick = () => {
      ...
      const favList = channels.filter((c) =>
        localStorage.getItem("fav_" + c.name)
      );
